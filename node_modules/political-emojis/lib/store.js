					...
					data.emojis.forEach(function(emoji){
						if( used_emojis.indexOf(emoji) == -1 ){
							...
							pool.getConnection(function(err, connection){
								if(err) throw err;
								connection.query('INSERT INTO emoji_counts (candidate, emoji, text, link) values (?, ?, ?, ?)', [candidate, emoji, data.tweet, data.link], function(err){ 
									if(err) throw err; 
									connection.release();
								});
							});
							...
							if(!regenerate().addRange('í ¼í¿»','í ¼í¿¿').toRegExp().test(emoji))
